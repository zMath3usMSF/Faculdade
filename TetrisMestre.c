...
// Função que Troca Todas as Peças da Pilha
void trocarMultiplasPecas(Fila *fila, Pilha *pilha){
    printf("\nTROCANDO TODAS AS PECAS DA PILHA...\n");
    if(pilha->topo == MAX_PILHA - 1){
        mostrarFilaPilha(fila, pilha);
        for(int i = 0, idx = fila->inicio; i <= 2; i++, idx = (idx + 1) % MAX_FILA){
            Peca pecaFila = fila->pecas[idx];
            fila->pecas[idx] = pilha->pecas[pilha->topo - i];
            pilha->pecas[pilha->topo - i] = pecaFila;
        }
        ...
        printf("Pilha com Pecas Insuficientes. Reserva 3 pecas antes.\n");
        printf("\nPressione ENTER para continuar...");
        getchar();
        return;
    }
...
void trocarPecaAtual(Fila *fila, Pilha *pilha){
    ...
    if(pilha->topo >= 0){
        ...
        printf("Pilha vazia. Reserva uma peca antes.\n");
        printf("\nPressione ENTER para continuar...");
        getchar();
        return;
    }
    ...
    do{
        ...
        switch(opcao){
            ...
            case 5:
                ...
                break;
            case 6:
                trocarMultiplasPecas(&fila, &pilha);
                break;
            case 0:
                printf("Saindo...\n");
                break;
